...
const deleteFireEvent = (report) => {
  if (confirm(`화재 #${report.id}를 오보로 처리하시겠습니까?`)) {
    activeReports.value = activeReports.value.filter(r => r.id !== report.id);
        ...
        <div class="p-4 border-b border-gray-200">
          <div class="flex justify-between items-center">
            ...
            <div class="flex items-center text-sm text-gray-500">
              <span>총 {{ activeReports.length + pendingReports.length }}개 제보</span>
            </div>
          </div>
        </div>
        
        <div class="p-4">
          <div v-if="activeReports.length + pendingReports.length === 0" class="text-center py-8 text-gray-500">
              ...
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  <div class="h-10 w-10 rounded-full bg-gray-200 overflow-hidden mr-3">
                    <img :src="report.metadata.imageUrl" :alt="report.id" class="h-full w-full object-cover" />
                  </div>
                  <div>
                    <div class="flex items-center">
                      <span 
